# =====================================================
# research_v06.py
# 岡畠AI研究所 Ver.0.6 ChatGPT版
# USD/JPY 5分足 条件研究ツール
#
# 実行方法:
#   python research_v06.py
#
# 出力先:
#   data/research_v06/
# =====================================================
...
class Config:
    ...
    out_dir: Path = Path(__file__).parent / "data" / "research_v06"
...
def calc_stats(pnl: np.ndarray, ts: np.ndarray) -> Dict[str, float]:
    """成績をまとめる。Ver0.6では分析用に期待値・総利益・総損失も出力する。"""
    if len(pnl) == 0:
        return {
            "取引回数": 0, "1日平均回数": 0.0, "勝率%": 0.0,
            "総損益pips": 0.0, "期待値pips/回": 0.0,
            "総利益pips": 0.0, "総損失pips": 0.0,
            "平均利益pips": 0.0, "平均損失pips": 0.0,
            "損益比": 0.0, "PF": 0.0,
    ...
    return {
        ...
        "期待値pips/回": round(float(pnl.mean()), 3),
        "総利益pips": round(gross_w, 1),
        "総損失pips": round(gross_l, 1),
        "平均利益pips": round(avg_win, 2),
        "平均損失pips": round(avg_loss, 2),
        "損益比": round(avg_win / abs(avg_loss), 3) if avg_loss < 0 else 999.0,
...
def build_report(rank: pd.DataFrame, adopted: pd.DataFrame, overfit: pd.DataFrame, cfg: Config, elapsed: float) -> str:
    L: List[str] = []
    L.append("=" * 62)
    L.append("岡畠AI研究所 条件研究レポート Ver.0.6 ChatGPT版")
    L.append("=" * 62)
    ...
    if adopted.empty:
        ...
        for i, (_, r) in enumerate(adopted.head(cfg.top_n_report).iterrows(), 1):
            L.append(f"{i}. {r['戦略']} TP{r['利確pips']} SL{r['損切りpips']} "
                     ...
                     f"期待値{r['検証_期待値pips/回']}pips/回 "
                     f"勝率{r['検証_勝率%']}% 取引{r['検証_取引回数']}回 スコア{r['総合スコア']}")
